...
path_remind <- normalizePath(".", mustWork = TRUE, winslash = "/")
path_magpie <-
  normalizePath("../magpie", mustWork = TRUE, winslash = "/")
...
path_remind_oldruns <- normalizePath(file.path(path_remind, "output"),
                                     winslash = "/",
                                     mustWork = FALSE)
path_magpie_oldruns <- normalizePath(file.path(path_magpie, "output"),
                                     winslash = "/",
                                     mustWork = FALSE)
...
scenarios_coupled  <-
  subset(scenarios_coupled, subset = (start == "1"))
if (length(grep("\\.", rownames(scenarios_coupled))) > 0)
  stop(
    "One or more titles contain dots - GAMS would not tolerate this, and quit working at a point where you least expect it. Stopping now. "
  )

missing <-
  setdiff(rownames(scenarios_coupled), rownames(settings_remind))
if (!identical(missing, character(0))) {
  cat(
    paste0(
      "The following scenarios are given in '",
      path_settings_coupled,
...
common <-
  intersect(rownames(settings_remind), rownames(scenarios_coupled))
if (!identical(common, character(0))) {
  cat("The following ",
      length(common),
      " scenarios will be started:\n")
  cat(common, sep = "\n")
}
...
for (scen in common) {
  ...
  # name of the run that is used for the folder names
  runname <- paste0(prefix_runname, scen)
  # sets the path to the report REMIND is started with in the first loop
  path_report <- NULL
  # set the SLURM quality of service (priority/short/medium/...)
  qos <- scenarios_coupled[scen, "qos"]
  # if qos could not be found in scenarios_coupled use short
  if (is.null(qos)) qos <- "short"
  ...
  suche <- file.path(
    path_remind_oldruns,
    paste0(prefix_oldruns, needle, "-rem-*/fulldata.gdx"))
  already_rem <- Sys.glob(suche)
  if (length(already_rem) == 0) cat("Nothing found for", suche, "\n")

  if (length(already_rem) > 0) {
    ...
    if (
      identical(
        normalizePath(file.path(path_remind, "output"),winslash = "/",mustWork = FALSE),
        path_remind_oldruns) &&
      identical(prefix_runname, prefix_oldruns) &&
      is.na(scenarios_coupled[scen, "oldrun"])
    ) {
      # continue counting only if remind is started in the same directory the gdxes are taken from
      iter_rem <- as.integer(sub(".*rem-(\\d.*)/.*", "\\1", already_rem))
    }

    # is there already a MAgPIE run with this name?
    suche <- file.path(path_magpie_oldruns,
                       paste0(prefix_oldruns, needle, "-mag-*/report.mif"))
    already_mag <- Sys.glob(suche)
    if (length(already_mag) == 0) cat("Nothing found for", suche, "\n")
    iter_mag <- 0
    if (length(already_mag) > 0) {
      already_mag <- mixedsort(already_mag)[1]
      path_report <- normalizePath(already_mag)
      cat(paste0(
        "Found MAgPIE report here: ",
        normalizePath(already_mag),
        "\n"
      ))
    ...
    if (iter_rem > iter_mag) {
      ...
      if (is.na(path_report))
        stop(
          "There is a fulldata.gdx but no REMIND_generic_.mif in ",
          path_run,
  ...
  if (!is.na(scenarios_coupled[scen, "path_report"])) {
    ...
    cat(
      "Replacing path to MAgPIE report with that one specified in\n  ",
      path_settings_coupled, "\n  ",
      scenarios_coupled[scen, "path_report"], "\n")
  }

  source(file.path(path_remind, "config/default.cfg")) # retrieve REMIND settings
  cfg_rem <- cfg
  rm(cfg)
  ...
  gdxlist <-
    c(
      input.gdx     = settings_remind[scen, "path_gdx"],
      # eventually this was updated if older runs exists in this folder (see above)
  ...
  cfg_rem$files2export$start <-
    .setgdxcopy(".gdx", cfg_rem$files2export$start, gdxlist)
  ...
  cfg_rem$RunsUsingTHISgdxAsInput <-
    settings_remind[common, ] %>%                 # select all scenarios that are going to be started
  ...
  start_now <-
    (substr(
      ...
      nchar(settings_remind[scen, "path_gdx_ref"])
    ) == ".gdx"
  ...
  if (has_carbonprice_path) {
    cp_start_now <-
      (substr(
        ...
        nchar(settings_remind[scen, "path_gdx_carbonprice"])
      ) == ".gdx"
      |
  ...
  cat("path_remind   : ",
      ifelse(dir.exists(path_remind), green, red),
      path_remind,
      NC,
      "\n",
      sep = "")
  cat("path_magpie   : ",
      ifelse(dir.exists(path_magpie), green, red),
      path_magpie,
      NC,
      "\n",
      sep = "")
  cat(
    "remind gdx    : ",
    ifelse(file.exists(cfg_rem$files2export$start["input.gdx"]), green, red),
    cfg_rem$files2export$start["input.gdx"],
    NC,
    "\n",
    sep = ""
  )
  cat(
    "ref_gdx       : ",
    ifelse(file.exists(cfg_rem$files2export$start["input_ref.gdx"]), green, red),
    cfg_rem$files2export$start["input_ref.gdx"],
    NC,
    "\n",
    sep = ""
  ...
  cat(
    "bau_gdx       : ",
    ifelse(file.exists(cfg_rem$files2export$start["input_bau.gdx"]), green, red),
    cfg_rem$files2export$start["input_bau.gdx"],
    NC,
    "\n",
    sep = ""
  ...
  if (has_carbonprice_path)
    cat(
      "carbonprice gdx : ",
      ifelse(file.exists(cfg_rem$files2export$start['input_carbonprice.gdx']), green, red),
      cfg_rem$files2export$start['input_carbonprice.gdx'],
      NC,
      "\n",
      sep = ""
  ...
  if (!is.null(path_mif_ghgprice_land))
    cat(
      "ghg_price_mag : ",
      ifelse(file.exists(path_mif_ghgprice_land), green, red),
  ...
  cat("path_report   : ",
      ifelse(file.exists(path_report), green, red),
      path_report,
      NC,
      "\n",
      sep = "")
  cat("no_ghgprices_land_until:",
      cfg_mag$mute_ghgprices_until,
      "\n")

  if (cfg_rem$gms$optimization == "nash" &&
      cfg_rem$gms$cm_nash_mode == "parallel") {
    # for nash: set the number of CPUs per node to number of regions + 1
    nr_of_regions <-
      length(unique(read.csv2(cfg_rem$regionmapping)$RegionCode)) + 1
  } else {
