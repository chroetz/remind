...
q_emiCdrAll(t,regi)..
  ...
  !! BECC + DACC
    (
      sum(emiBECCS2te(enty,enty2,te,enty3), vm_emiTeDetail(t,regi,enty,enty2,te,enty3)) !! BECCS
      + sum(teCCS2rlf(te,rlf), vm_ccs_cdr(t,regi,"cco2","ico2","ccsinje",rlf)) !! DAC
  ...
  !! emissions stored in biochar
    + vm_emiTeDetail(t,regi,"pebiolc","seliqbio","biochar","cco2")
  !! net negative emissions from co2luc  
    - p_macBaseMagpieNegCo2(t,regi)
  !! negative emissions from the cdr module that are not stored geologically, i.e. EW
    - 
    (
      vm_emiCdr(t,regi,"co2")  !! v33_emiEW(t,regi) + v33_emiDAC(t,regi)
      + sum(teCCS2rlf(te,rlf), vm_ccs_cdr(t,regi,"cco2","ico2","ccsinje",rlf)) !! = -v33_emiDAC(t,regi)
    )
;
